...
final class ConfiguracaoContrato extends ConfiguracaoAbstract implements ConfiguracaoInterface
{
    ...
    public function actionLoadContrato()
    {
        ...
        foreach ($licitacaoList as $contrato) {
            $rendered = [];
        ...
        if ('POST' === $this->getRequest()->getMethod()) {
            ...
            if (true === $form->isValid()) {
                ...
                if (0 === count($message)) {
                    ...
                    foreach ($contrato->getFkTcemgContratoEmpenhos() as $empenho) {
                        $empenho->setCodContrato($contrato->getCodContrato());
                    }

                    foreach ($contrato->getFkTcemgContratoFornecedores() as $fornecedor) {
                        $fornecedor->setCodContrato($contrato->getCodContrato());
                    }
